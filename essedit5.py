...
FileHeader = namedtuple('FileHeader', 'fileId headerSize majorVersion minorVersion exeTime')
...
def load(filename, imagename=None):
    with open(filename, 'rb') as essfile:
        # FileHeader
        headerpart = list(unpack('13s H I I', essfile.read(24)))
        print headerpart
        print parse_b_or_bzstring(essfile, bz=True)
        namelen, = unpack('H', essfile.read(2))
        name = essfile.read(namelen)
        print namelen
        print name
